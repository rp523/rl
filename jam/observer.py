...
def observe(agents, agent_idx, map_h, map_w, pcpt_h, pcpt_w):
    ...
    for wall_y0, wall_x0, wall_y1, wall_x1 in [
        ...
        obs_py0 = int((obs_y0 + own_obs_y) / pcpt_yrange * pcpt_h + 0.5)
        obs_py1 = int((obs_y1 + own_obs_y) / pcpt_yrange * pcpt_h + 0.5)
        obs_px0 = int((obs_x0 + own_obs_x) / pcpt_xrange * pcpt_w + 0.5)
        obs_px1 = int((obs_x1 + own_obs_x) / pcpt_xrange * pcpt_w + 0.5)
    ...
    for other in (agents[:agent_idx] + agents[agent_idx + 1:]):
        ...
        obs_py0 = int((obs_y - other.radius_m + own_obs_y) / pcpt_yrange * pcpt_h + 0.5)
        obs_py1 = int((obs_y + other.radius_m + own_obs_y) / pcpt_yrange * pcpt_h + 0.5)
        obs_px0 = int((obs_x - other.radius_m + own_obs_x) / pcpt_xrange * pcpt_w + 0.5)
        obs_px1 = int((obs_x + other.radius_m + own_obs_x) / pcpt_xrange * pcpt_w + 0.5)
    ...
    obs_y, obs_x = rotate(rel_y, rel_x, 0.5 * onp.pi - own.theta)
    obs_py0 = int((obs_y - own.radius_m + own_obs_y) / pcpt_yrange * pcpt_h + 0.5)
    obs_py1 = int((obs_y + own.radius_m + own_obs_y) / pcpt_yrange * pcpt_h + 0.5)
    obs_px0 = int((obs_x - own.radius_m + own_obs_x) / pcpt_xrange * pcpt_w + 0.5)
    obs_px1 = int((obs_x + own.radius_m + own_obs_x) / pcpt_xrange * pcpt_w + 0.5)
    if  (obs_py0 <= pcpt_h - 1) and (0 <= obs_py1) and \
        (obs_px0 <= pcpt_w - 1) and (0 <= obs_px1):
        state[0, max(obs_py0, 0) : min(obs_py1 + 1, pcpt_h), max(obs_px0, 0) : min(obs_px1 + 1, pcpt_w), EnChannel.occupy] = - 1.0
    else:
